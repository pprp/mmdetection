...
model = dict(
    ...
    backbone=dict(
        ...
        norm_eval=True,
        style='pytorch',
        plugins=[dict(cfg=dict(type='ReceptiveFieldAttention'),
    ...
    bbox_head=dict(
        type='SSDHead',
        in_channels=(1024, 2048, 512, 256, 256, 256),
        num_classes=80,
        use_depthwise=True,
        norm_cfg=dict(type='BN', eps=0.001, momentum=0.03),
        act_cfg=dict(type='ReLU6'),
...
data_root = '/data/public/coco2017/'
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type=dataset_type,
            ann_file="/data/home/scv6681/run/data/subcoco/instances_minitrain2017.json", #ata_root + 'annotations/instances_train2017.json',
            img_prefix=data_root + 'train2017/',
            pipeline=train_pipeline)),
    val=dict(pipeline=test_pipeline),
    test=dict(pipeline=test_pipeline))

# optimizer for 4 GPUS lr=0.002 failed 
# optimizer for 4 GPUS 
optimizer = dict(type='SGD', lr=0.0075, momentum=0.9, weight_decay=4.0e-5)
